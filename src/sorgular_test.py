...
SORGULAR = [
    ("SORGU 1: Tür ve Irka Göre Hastalık Dağılımı", """
    ...
    ("SORGU 2: Aşı Takvimi Gecikmiş Hayvanlar", """
        SELECT h.ad AS hayvan_adi, h.tur, h.irk,
               k.ad || ' ' || k.soyad AS sahip_adi,
               k.telefon AS sahip_tel, a.asi_turu,
    ...
    ("SORGU 3: Veteriner Hekim Aylık Performans", """
        SELECT k.ad || ' ' || k.soyad AS veteriner_adi,
               v.uzmanlik_alani,
    ...
    ("SORGU 4: Kritik Stok ve SKT Raporu", """
        SELECT i.ilac_adi, i.etken_madde, i.kategori,
               i.stok_miktari, i.kritik_stok_seviyesi,
               i.son_kullanma_tarihi, t.firma_adi AS tedarikci,
               CASE
        ...
        ORDER BY
            CASE
                ...
                ELSE 3
            END, i.son_kullanma_tarihi;
    """),

    ("SORGU 5: Hizmet Türüne Göre Aylık Gelir Analizi", """
        SELECT ay, hizmet_adi, toplam_gelir, fatura_sayisi,
    ...
    ("SORGU 6: Hayvan Tedavi Geçmişi (Top 15)", """
        SELECT h.ad AS hayvan_adi, h.tur, h.irk,
    ...
    ("SORGU 8: Ödenmemiş Fatura Takip", """
        SELECT f.fatura_id, f.tarih::DATE AS fatura_tarihi,
               k.ad || ' ' || k.soyad AS sahip_adi,
               k.telefon, k.e_posta, f.toplam_tutar,
    ...
    ("SORGU 10: Anormal Lab Değerleri", r"""
        SELECT lt.test_id, h.ad AS hayvan_adi, h.tur,
               lt.test_turu, lt.sonuc::DECIMAL(10,2) AS sonuc_deger,
               lt.referans_deger_alt, lt.referans_deger_ust, lt.birim,
...
def sorgu_calistir(conn, baslik, sql):
    ...
    try:
        ...
        if sonuclar:
            print(f"\n📊 {len(sonuclar)} sonuç:\n")
            kisaltilmis = []
            for row in sonuclar[:25]:
                kisaltilmis_row = []
                for val in row:
                    if isinstance(val, str) and len(val) > 30:
                        kisaltilmis_row.append(val[:27] + "...")
                    else:
                        kisaltilmis_row.append(val)
                kisaltilmis.append(kisaltilmis_row)

            print(tabulate(kisaltilmis, headers=kolonlar,
